...
function App() {
  ...
  return (
    <>
      ...
      {isFrontPage ? (
        <VideoComp playVideoLogic={playVideo} className="App"></VideoComp>
      ) : (
        <HomeBtn homeBtnLogic={homeBtn} />
      )}
      {isInfoRouting ? (
        <InfoPage
          moveToParticularInfo={moveToParticularInfo}
          homeBtnLogic={homeBtn}
        />
      ) : null}
      {isTextInsert ? <TextInserter /> : null}
      {isParticularInfoPage ? (
        <ParticularInfo
          backBtnLogic={backToInfoRouting}
          homeBtnLogic={homeBtn}
          typeOfParticularInfo={typeOfParticularInfo}
        />
      ) : null}
    </>
  );
